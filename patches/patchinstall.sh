...
usage()
{
	...
	echo "  --backend=dummy      Only generate list of patches"
	echo ""
}
...
# dummy backend - only generate list of patches (mainly for developers)
elif test "$backend" = "dummy"; then

	# Only list the regular patches, no autogenerated ones -
	# moreover, don't run autoreconf or ./tools/make_requests.
	enable_patchlist=0
	enable_autoconf=0
	...
	patch_apply_file()
	{
		_nesting=$(echo "${patchdir}/" | tr -d -c / | wc -c)
		_shortname=$(echo "$1" | cut -d / -f ${_nesting}-)
		echo "${_shortname}"
		unset _nesting _shortname
	}
